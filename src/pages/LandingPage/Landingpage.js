    ...
    >
      <div className="Underpart h-1/2 w-full ">
        <div className="title h-1/2  uppercase flex flex-row justify-start items-center bg-red font-Bebas_Neue  text-6xl ">
          <h1 className="text-white px-8 sm: w-fit tracking-widest md:text-7xl text-3xl ">
            {title}
          </h1>
          {logo == true && (
            <img
              className=" h-full w-2/5  md:w-2/4 object-cover    "
              src={Scorelogo}
            ></img>
          )}
        </div>
        <div className="description  flex flex-col h-1/2 px-8 text-white/80  md:text-xl sd:text-lg text-base justify-center uppercase tracking-wider   ">
          <h2>{description}</h2>
        </div>
      </div>
    </div>
